...
auth.onAuthStateChanged(async (user) => {
  if (!user) {
    location.replace('index.html'); // tu flujo actual
    return;
  }
  ...
  try{
    ...
    snap.forEach(doc => {
      ...
      rows.push({
        ...
        source: d.source || '',
        // campos de recurrencia / permanencia (si existen en tus docs)
...
btnExport?.addEventListener('click', () => {
  ...
  const phoneFmt = (s) => s.replace(/[^\d]/g,''); // solo dígitos
  ...
  const headerStyle = {
    ...
    fill: { patternType: "solid", fgColor: { rgb: "FF116E09" } }, // tu "brand-red" verde
  ...
  for (let r = 1; r < wsData.length; r++){
    ...
    for (let c = 0; c < HEAD.length; c++){
      ...
      if (c === 2) { ws[addr].z = '00000000000'; } // teléfono
      if (c === 4 || c === 7) {
  ...
  for (let r = 4; r <= 7; r++){
    ...
    ws2[`A${r}`].s = { ...zebra, border: border2 };
    ws2[`B${r}`].s = { ...zebra, border: border2 };
    if (r !== 7) ws2[`B${r}`].t = 'n';
  }
